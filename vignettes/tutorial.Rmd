...
`formula`: The formula of interests. It should follow the format of y~x. You can add more than one explanatory variables with different form like y~x+z+z^2.
...
## The Use of Function `linear_model`
...
system.time(lm(testz~testx + testy + testy^2))
system.time(linear_model(testz~testx + testy + testy^2))
```
...
linear_model(testy~testx, digit = 5)
lm(testy~testx)
```
